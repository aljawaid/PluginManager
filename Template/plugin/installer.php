<fieldset class="manual-install">
    ...
    <form method="post" class="file-url-form" action="<?=$this->url->href('PluginManagerController', 'installPlugin', array('plugin' => 'PluginManager'))?>" enctype="multipart/form-data" autocomplete="on" >
        ...
        <section class="install-section">
            ...
            <?= $this->form->input('file', 'plugin_file', array(), array(), array('id="form-pluginfile" accept="application/zip"'), 'plugin-file') ?>
        </section>
        <section class="install-section">
            <?= $this->form->label(t('From URL'), 'plugin_url') ?>
            <?= $this->form->input('url', 'plugin_url', array(), array(), array('id="plugin_url" placeholder="https://mydomain.com/plugin.zip"'), 'plugin-url') ?>
        </section>
